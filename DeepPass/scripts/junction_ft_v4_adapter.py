...
class AdapterWrappedLayer(nn.Module):
    ...
    Proxies all attributes to the original layer so the model's forward pass
    can access layer_type, attention_type, etc."""
    ...
    def __getattr__(self, name):
        # First check our own attributes (layer, adapter, etc.)
        try:
            return super().__getattr__(name)
        except AttributeError:
            # Proxy to the wrapped layer
