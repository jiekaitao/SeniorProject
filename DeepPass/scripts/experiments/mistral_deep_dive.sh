#!/bin/bash
#SBATCH --partition=hpg-b200
#SBATCH --gpus=1
#SBATCH --cpus-per-task=2
#SBATCH --mem=16G
#SBATCH --time=06:00:00
#SBATCH --account=cis4914
#SBATCH --qos=cis4914
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_mistral_deep_%j.log
#SBATCH --job-name=mis_deep

# Mistral 7B Deep Dive — WHY does this model improve on lm-eval?
# The ONLY model showing benchmark improvement (+0.7% BBH, +1.0% MATH, +2.6% MuSR)
# Tests: attn-only, whisper FFN, pair search, gate margin, sublayer lm-eval

cd /blue/cis4914/jietao/DeepPass
export LD_PRELOAD=/blue/cis4914/jietao/DeepPass/envs/deeppass/lib/libstdc++.so.6
PYTHON=/blue/cis4914/jietao/DeepPass/envs/deeppass/bin/python

echo "=== Mistral 7B Deep Dive ==="
echo "Started: $(date)"

$PYTHON -c "
import os, sys, json, time, gc, torch, torch.nn as nn, functools, numpy as np
os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'expandable_segments:True'
sys.path.insert(0, 'scripts')
from transformers import AutoModelForCausalLM, AutoTokenizer
from math_probe import run_math_probe
from eq_bench_probe import run_eq_bench_probe, _load_questions

MODEL_ID = 'mistralai/Mistral-7B-Instruct-v0.3'
CACHE_DIR = '/blue/cis4914/jietao/hf_cache'
TASKS = 'leaderboard_bbh,leaderboard_math_hard,leaderboard_musr,leaderboard_mmlu_pro'
LIMIT = 0.15
SAVE_DIR = 'results/data/mistral_7b'
os.makedirs(f'{SAVE_DIR}/lm_eval', exist_ok=True)

class LayerIdxWrapper(nn.Module):
    def __init__(self, layer, new_idx):
        super().__init__()
        self.layer = layer
        self.new_layer_idx = new_idx
        self.orig_idx = layer.layer_idx
        self.orig_attn = layer.self_attn.layer_idx if hasattr(layer, 'self_attn') else None
    def forward(self, *args, **kwargs):
        self.layer.layer_idx = self.new_layer_idx
        if hasattr(self.layer, 'self_attn'): self.layer.self_attn.layer_idx = self.new_layer_idx
        try: return self.layer(*args, **kwargs)
        finally:
            self.layer.layer_idx = self.orig_idx
            if self.orig_attn is not None: self.layer.self_attn.layer_idx = self.orig_attn
    def __getattr__(self, name):
        try: return super().__getattr__(name)
        except AttributeError: return getattr(self.layer, name)

def build_order(blocks, N):
    s = sorted(blocks); order, prev = [], 0
    for (i, j) in s:
        order.extend(range(prev, j)); order.extend(range(i, j)); prev = j
    order.extend(range(prev, N)); return order

def apply_dup(model, blocks):
    inner = model.model; orig = list(inner.layers); N = len(orig)
    order = build_order(blocks, N)
    seen = set(); is_dup = []
    for idx in order: is_dup.append(idx in seen); seen.add(idx)
    new_layers = []
    for pi, (oi, d) in enumerate(zip(order, is_dup)):
        l = orig[oi]
        if d: new_layers.append(LayerIdxWrapper(l, pi))
        else:
            l.layer_idx = pi
            if hasattr(l, 'self_attn'): l.self_attn.layer_idx = pi
            new_layers.append(l)
    inner.layers = nn.ModuleList(new_layers)
    model.config.num_hidden_layers = len(new_layers)
    model.config.use_cache = True
    return orig, N

def apply_dup_with_sublayer(model, blocks, sublayer_alphas):
    orig, N = apply_dup(model, blocks)
    hooks = []
    inner = model.model; orig_layers_list = list(inner.layers)
    sorted_blocks = sorted(blocks)
    dup_layers = []
    for (i, j) in sorted_blocks:
        for l in range(i, j): dup_layers.append(l)

    # Need to get the ORIGINAL layer objects for hooking
    orig_layer_objs = [orig[idx] for idx in range(N)]

    for layer_idx in dup_layers:
        if layer_idx not in sublayer_alphas: continue
        attn_a, ffn_b = sublayer_alphas[layer_idx]
        module = orig_layer_objs[layer_idx]

        if abs(attn_a - 1.0) > 1e-6:
            actr = [0]; aa = attn_a
            def make_ah(c, a):
                def hook(mod, inp, out):
                    c[0] += 1
                    if c[0] % 2 == 0:
                        return (a * out[0],) + out[1:] if isinstance(out, tuple) else a * out
                    return out
                return hook
            hooks.append(module.self_attn.register_forward_hook(make_ah(actr, aa)))

        if abs(ffn_b - 1.0) > 1e-6:
            fctr = [0]; fb = ffn_b
            def make_fh(c, b):
                def hook(mod, inp, out):
                    c[0] += 1
                    if c[0] % 2 == 0:
                        return (b * out[0],) + out[1:] if isinstance(out, tuple) else b * out
                    return out
                return hook
            hooks.append(module.mlp.register_forward_hook(make_fh(fctr, fb)))

    return orig, N, hooks

def restore(model, orig, N):
    model.model.layers = nn.ModuleList(orig)
    model.config.num_hidden_layers = N

def save(data, name):
    with open(f'{SAVE_DIR}/{name}.json', 'w') as f:
        json.dump(data, f, indent=2)
    print(f'  [SAVED] {name}.json', flush=True)

BEST_BLOCK = [(28, 29)]
eq_all = _load_questions()

# ======================================================================
# Part 1: Sublayer dual-probe analysis
# ======================================================================
print('\\n=== Part 1: Sublayer Dual-Probe Analysis ===', flush=True)

tokenizer = AutoTokenizer.from_pretrained(MODEL_ID, cache_dir=CACHE_DIR, trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained(MODEL_ID, cache_dir=CACHE_DIR, device_map='auto', dtype=torch.bfloat16, trust_remote_code=True)
inner = model.model; original_layers = list(inner.layers); N = len(original_layers); device = next(model.parameters()).device

def gen(p):
    inp = tokenizer(p, return_tensors='pt', truncation=True, max_length=256)['input_ids'].to(device)
    with torch.no_grad():
        out = model.generate(inp, max_new_tokens=64, do_sample=False, use_cache=True)
    return tokenizer.decode(out[0][inp.shape[1]:], skip_special_tokens=True)
def gen_long(p):
    inp = tokenizer(p, return_tensors='pt', truncation=True, max_length=256)['input_ids'].to(device)
    with torch.no_grad():
        out = model.generate(inp, max_new_tokens=128, do_sample=False, use_cache=True)
    return tokenizer.decode(out[0][inp.shape[1]:], skip_special_tokens=True)

# Baseline
math_r = run_math_probe(gen, verbose=False)
eq_r = run_eq_bench_probe(gen_long, questions=eq_all, verbose=False)
baseline = math_r['score'] * 50 + eq_r['score'] * 0.5
print(f'Baseline: {baseline:.2f}', flush=True)

# Full dup
orig, oN = apply_dup(model, BEST_BLOCK)
math_r = run_math_probe(gen, verbose=False)
eq_r = run_eq_bench_probe(gen_long, questions=eq_all, verbose=False)
full_dup = math_r['score'] * 50 + eq_r['score'] * 0.5
print(f'Full dup (28,29): {full_dup:.2f} (+{full_dup-baseline:.2f})', flush=True)
restore(model, orig, oN)

# Attn-only
orig, oN, hooks = apply_dup_with_sublayer(model, BEST_BLOCK, {28: (1.0, 0.0)})
math_r = run_math_probe(gen, verbose=False)
eq_r = run_eq_bench_probe(gen_long, questions=eq_all, verbose=False)
attn_only = math_r['score'] * 50 + eq_r['score'] * 0.5
for h in hooks: h.remove()
restore(model, orig, oN)
print(f'Attn-only: {attn_only:.2f} (FFN impact: {attn_only - full_dup:+.2f})', flush=True)

# Whisper FFN
orig, oN, hooks = apply_dup_with_sublayer(model, BEST_BLOCK, {28: (1.0, 0.2)})
math_r = run_math_probe(gen, verbose=False)
eq_r = run_eq_bench_probe(gen_long, questions=eq_all, verbose=False)
whisper = math_r['score'] * 50 + eq_r['score'] * 0.5
for h in hooks: h.remove()
restore(model, orig, oN)
print(f'Whisper FFN: {whisper:.2f}', flush=True)

# Gate margin
orig, oN = apply_dup(model, BEST_BLOCK)
module = original_layers[28]
gate_proj = module.mlp.gate_proj
cal_prompts = ['What is 127 * 348?', 'What is the capital of France?', 'Explain entropy simply.', 'What is 2^16?', 'Who wrote Romeo and Juliet?', 'What is the square root of 152399025?']
flips = []
for prompt in cal_prompts:
    gv, ctr = [], [0]
    def make_cap(c, g, gp):
        def hook(mod, inp, out):
            c[0] += 1
            with torch.no_grad():
                i = inp[0] if isinstance(inp, tuple) else inp
                g.append(gp(i[:, -1, :]).cpu().float().squeeze(0))
            return out
        return hook
    h = module.mlp.register_forward_hook(make_cap(ctr, gv, gate_proj))
    inputs = tokenizer(prompt, return_tensors='pt', truncation=True, max_length=64).to(device)
    with torch.no_grad(): model(inputs['input_ids'], use_cache=False)
    h.remove()
    if len(gv) >= 2:
        flips.append(((gv[0] > 0) != (gv[1] > 0)).float().mean().item())
restore(model, orig, oN)
avg_flip = float(np.mean(flips))
print(f'Gate flip L28: {avg_flip:.4f}', flush=True)

save({
    'baseline': baseline, 'full_dup': full_dup,
    'attn_only': attn_only, 'whisper_ffn02': whisper,
    'ffn_impact': attn_only - full_dup,
    'gate_flip_L28': avg_flip,
}, 'deep_dive_sublayer')

del model, tokenizer; gc.collect(); torch.cuda.empty_cache()

# ======================================================================
# Part 2: Sublayer lm-eval (attn-only and whisper on benchmarks)
# ======================================================================
print('\\n=== Part 2: Sublayer lm-eval ===', flush=True)

configs_lm = [
    ('attn_only_28_29', BEST_BLOCK, {28: (1.0, 0.0)}),
    ('whisper_ffn02_28_29', BEST_BLOCK, {28: (1.0, 0.2)}),
]

for config_name, blocks, sublayer_alphas in configs_lm:
    outpath = f'{SAVE_DIR}/lm_eval/{config_name}.json'
    if os.path.exists(outpath):
        print(f'Skipping {config_name}', flush=True); continue

    print(f'\\n  Config: {config_name}', flush=True)
    tokenizer = AutoTokenizer.from_pretrained(MODEL_ID, cache_dir=CACHE_DIR, trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained(MODEL_ID, cache_dir=CACHE_DIR, device_map='auto', dtype=torch.bfloat16, trust_remote_code=True)

    orig, oN, hooks = apply_dup_with_sublayer(model, blocks, sublayer_alphas)

    from lm_eval.models.huggingface import HFLM; from lm_eval import evaluator
    lm = HFLM(pretrained=model, tokenizer=tokenizer, batch_size=1)

    t0 = time.time()
    try:
        results = evaluator.simple_evaluate(model=lm, tasks=TASKS.split(','), limit=LIMIT)
        elapsed = time.time() - t0
        scores = {f'{t}/{m}': v for t, d in results['results'].items() for m, v in d.items() if isinstance(v, (int, float))}
        print(f'  RESULTS ({config_name}) [{elapsed:.0f}s]', flush=True)
        for k, v in sorted(scores.items()):
            if 'stderr' not in k and k.count('/') == 1: print(f'    {k}: {v:.4f}', flush=True)
        with open(outpath, 'w') as f:
            json.dump({'config': config_name, 'blocks': [list(b) for b in blocks],
                       'sublayer_alphas': {str(k): list(v) for k, v in sublayer_alphas.items()},
                       'scores': scores, 'elapsed_s': elapsed}, f, indent=2)
        print(f'  SAVED {outpath}', flush=True)
    except Exception as e:
        print(f'  FAILED: {e}', flush=True); import traceback; traceback.print_exc()

    for h in hooks: h.remove()
    del model, tokenizer, lm; gc.collect(); torch.cuda.empty_cache()

# ======================================================================
# Part 3: Greedy pair search (dual probe)
# ======================================================================
print('\\n=== Part 3: Greedy Pair Search ===', flush=True)

tokenizer = AutoTokenizer.from_pretrained(MODEL_ID, cache_dir=CACHE_DIR, trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained(MODEL_ID, cache_dir=CACHE_DIR, device_map='auto', dtype=torch.bfloat16, trust_remote_code=True)
inner = model.model; original_layers = list(inner.layers); N = len(original_layers); device = next(model.parameters()).device

def gen2(p):
    inp = tokenizer(p, return_tensors='pt', truncation=True, max_length=256)['input_ids'].to(device)
    with torch.no_grad():
        out = model.generate(inp, max_new_tokens=64, do_sample=False, use_cache=True)
    return tokenizer.decode(out[0][inp.shape[1]:], skip_special_tokens=True)
def gen2_long(p):
    inp = tokenizer(p, return_tensors='pt', truncation=True, max_length=256)['input_ids'].to(device)
    with torch.no_grad():
        out = model.generate(inp, max_new_tokens=128, do_sample=False, use_cache=True)
    return tokenizer.decode(out[0][inp.shape[1]:], skip_special_tokens=True)

pairs = []
for start in range(0, N-1, 3):
    cand = (start, start+1)
    if not (cand[1] <= 28 or 29 <= cand[0]): continue  # overlaps anchor
    blocks = [(28, 29), cand]
    orig, oN = apply_dup(model, blocks)
    math_r = run_math_probe(gen2, verbose=False)
    eq_r = run_eq_bench_probe(gen2_long, verbose=False)
    combined = math_r['score'] * 50 + eq_r['score'] * 0.5
    restore(model, orig, oN)
    print(f'  +(28,29)+({start},{start+1}): combined={combined:.2f}', flush=True)
    pairs.append({'blocks': [[28,29],[start,start+1]], 'combined': combined})

pairs.sort(key=lambda x: x['combined'], reverse=True)
print(f'\\nTop 3 pairs:', flush=True)
for p in pairs[:3]:
    print(f'  {p[\"blocks\"]}: {p[\"combined\"]:.2f}', flush=True)

save({'anchor': [28,29], 'pairs': pairs}, 'pair_search')
del model, tokenizer; gc.collect(); torch.cuda.empty_cache()

# ======================================================================
# Summary
# ======================================================================
print(f'\\n{\"=\" * 50}', flush=True)
print('SUMMARY — Mistral 7B Deep Dive', flush=True)
print(f'Baseline: {baseline:.2f}', flush=True)
print(f'Full dup (28,29): {full_dup:.2f} (+{full_dup-baseline:.2f})', flush=True)
print(f'Attn-only: {attn_only:.2f} (FFN impact: {attn_only-full_dup:+.2f})', flush=True)
print(f'Whisper: {whisper:.2f}', flush=True)
print(f'Gate flip L28: {avg_flip:.4f}', flush=True)
if pairs: print(f'Best pair: {pairs[0][\"blocks\"]} = {pairs[0][\"combined\"]:.2f}', flush=True)
print('COMPLETE', flush=True)
"

echo "=== Finished: $(date) ==="
