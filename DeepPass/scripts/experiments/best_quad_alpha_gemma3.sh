...
#SBATCH --qos=cis4914
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_best_quad_alpha_g3_%j.log
#SBATCH --job-name=g3_bqa

# Alpha-tune the ACTUAL best quad: (0,2)+(12,13)+(22,25)+(47,48)
# The greedy search found +(22,25)@0.2-0.3 = 89.70
# 6 params: L0, L1, L12, L22, L23, L24, L47 — wait, (22,25) = layers 22,23,24
# Total: 7 alpha params

cd /blue/cis4914/jietao/DeepPass
export LD_PRELOAD=/blue/cis4914/jietao/DeepPass/envs/deeppass/lib/libstdc++.so.6
PYTHON=/blue/cis4914/jietao/DeepPass/envs/deeppass/bin/python

echo "=== Best Quad Alpha: (0,2)+(12,13)+(22,25)+(47,48) ==="
echo "Started: $(date)"
...
BLOCKS = [(0, 2), (12, 13), (22, 25), (47, 48)]
SAVE_DIR = 'results/data/gemma3_27b/best_quad_alpha'
os.makedirs(SAVE_DIR, exist_ok=True)

# 7 alpha params: L0, L1, L12, L22, L23, L24, L47
ALPHA_NAMES = ['alpha_L0', 'alpha_L1', 'alpha_L12', 'alpha_L22', 'alpha_L23', 'alpha_L24', 'alpha_L47']
...
for idx, layer_idx in enumerate(dup_layers):
    ...
    hooks.append(h)
print(f'Applied: {N} -> {len(order)} layers, {len(hooks)} alpha hooks, dup_layers={dup_layers}', flush=True)
...
print(f'Sanity (all 1.0): combined={baseline:.2f} ({time.time()-t0:.0f}s)', flush=True)

study = optuna.create_study(
    study_name='gemma3_best_quad_alpha',
...
# Seed: all 1.0, triple alphas + whisper, and the known-good 0.2/0.3 for (22,25)
study.enqueue_trial({n: 1.0 for n in ALPHA_NAMES})
study.enqueue_trial({
    'alpha_L0': 0.88, 'alpha_L1': 0.81, 'alpha_L12': 1.45,
    'alpha_L22': 0.3, 'alpha_L23': 0.3, 'alpha_L24': 0.3, 'alpha_L47': 0.95,
})
study.enqueue_trial({
    'alpha_L0': 0.88, 'alpha_L1': 0.81, 'alpha_L12': 1.45,
    'alpha_L22': 0.2, 'alpha_L23': 0.2, 'alpha_L24': 0.2, 'alpha_L47': 0.95,
})
...
def objective(trial):
    ...
    a_str = ' '.join(f'{params[n]:.2f}' for n in ALPHA_NAMES)
...
for t in completed[:3]:
    ...
    combined = math_r['score'] * 50 + eq_r['score'] * 0.5
    elapsed = time.time() - t0
    print(f'  trial {t.number}: search={t.value:.2f} validated={combined:.2f} ({elapsed:.0f}s)', flush=True)
    val_results.append({
