...
#SBATCH --qos=cis4914
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_pla_quad_%j.log
#SBATCH --job-name=deeppass_plaq

# Per-LAYER alpha optimization for the FULL QUAD: (0,7)+(15,20)+(20,27)+(45,52)
# 28 total alphas: 7 + 5 + 7 + 7.
#
# Starting points (known good per-block alphas):
#   (0,7)   all 7 layers at 0.9
#   (15,20) all 5 layers at 0.1
#   (20,27) all 7 layers at 0.15
#   (45,52) optimized: [1.1, 1.0, 0.5, 1.3, 1.0, 0.9, 1.1]
#
# Coordinate descent: sweep each of the 28 layer-alphas one at a time.
# Up to 2 iterations (28 alphas x ~7 values each is a lot of evals).

cd /blue/cis4914/jietao/DeepPass
export LD_PRELOAD=/blue/cis4914/jietao/DeepPass/envs/deeppass/lib/libstdc++.so.6
PYTHON=/blue/cis4914/jietao/DeepPass/envs/deeppass/bin/python

echo "=== Per-Layer Alpha Quad Optimization ==="
echo "Started: $(date)"
...
print('=' * 70)
print('PER-LAYER ALPHA OPTIMIZATION — FULL QUAD')
print('Blocks: (0,7) + (15,20) + (20,27) + (45,52) = 28 layer-alphas')
...
SWEEP_RANGES = {
    (0, 7):   [0.0, 0.3, 0.5, 0.7, 0.9, 1.0, 1.1],
    (15, 20): [0.0, 0.05, 0.1, 0.15, 0.2, 0.3],
...
for offset in range(BLOCK_SIZES[(0, 7)]):
    initial_layer_alphas[((0, 7), offset)] = 0.9
for offset in range(BLOCK_SIZES[(15, 20)]):
    initial_layer_alphas[((15, 20), offset)] = 0.1
for offset in range(BLOCK_SIZES[(20, 27)]):
    initial_layer_alphas[((20, 27), offset)] = 0.15
for offset in range(BLOCK_SIZES[(45, 52)]):
    initial_layer_alphas[((45, 52), offset)] = INIT_ALPHAS_45_52[offset]

print('Initial per-layer alphas:', flush=True)
for block in BLOCKS:
    sz = BLOCK_SIZES[block]
    vals = [initial_layer_alphas[(block, i)] for i in range(sz)]
...
r = evaluate(BLOCKS, initial_layer_alphas, 'initial: (0,7)@0.9 (15,20)@0.1 (20,27)@0.15 (45,52)@optimized')
...
# =====================================================================
# COORDINATE DESCENT: sweep each of 28 layer-alphas
# =====================================================================
print(f'\n{\"=\" * 70}')
print('COORDINATE DESCENT: 28 layer-alphas')
print('Order: (0,7) L0-L6, (15,20) L0-L4, (20,27) L0-L6, (45,52) L0-L6')
print(f'{\"=\" * 70}', flush=True)
...
for iteration in range(2):
    ...
    for block in BLOCKS:
        ...
        print(f'\n  Block ({block[0]},{block[1]}), size={block_size}, sweep range: {sweep_range}', flush=True)
    ...
    for block in BLOCKS:
        sz = BLOCK_SIZES[block]
        vals = [best_la[(block, i)] for i in range(sz)]
        print(f'    ({block[0]},{block[1]}): {vals}', flush=True)
...
print(f'\nOptimized per-layer alphas (28 total):', flush=True)
for block in BLOCKS:
    sz = BLOCK_SIZES[block]
    vals = [best_la[(block, i)] for i in range(sz)]
    print(f'  ({block[0]},{block[1]}): {vals}', flush=True)
...
# Save results
os.makedirs('results/data/72b/per_layer_alpha_quad', exist_ok=True)
save_data = {
    'date': datetime.now().isoformat(),
    'blocks': [[b[0], b[1]] for b in BLOCKS],
    'block_sizes': {f'({b[0]},{b[1]})': BLOCK_SIZES[b] for b in BLOCKS},
    'initial_alphas': {f'({b[0]},{b[1]})_L{i}': initial_layer_alphas[(b, i)]
                       for b in BLOCKS for i in range(BLOCK_SIZES[b])},
    'best_alphas': {f'({b[0]},{b[1]})_L{i}': best_la[(b, i)]
                    for b in BLOCKS for i in range(BLOCK_SIZES[b])},
    'best_alphas_by_block': {
        f'({b[0]},{b[1]})': [best_la[(b, i)] for i in range(BLOCK_SIZES[b])]
        for b in BLOCKS
    },
...
with open('results/data/72b/per_layer_alpha_quad/results.json', 'w') as f:
    json.dump(save_data, f, indent=2)
print(f'\nSaved to results/data/72b/per_layer_alpha_quad/results.json', flush=True)
"
