#!/bin/bash
#SBATCH --job-name=dual_inj
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_dual_inject_%j.log
...
# Try injecting at two layers (duplicate the thoughts across 2 positions)
echo "=== DUAL-LAYER INJECTION: L14+L16, L12+L16, L12+L14 ==="
for COMBO in "14,16" "12,16" "12,14" "10,16" "8,16"; do
    for SEED in 42 7; do
        $PYTHON -c "
import sys, os
...
from eval_deliberation_creative import get_choice_token_ids, RESULTS_DIR, CHOICE_MAP

device = torch.device('cuda')

class DualLayerDeliberation(RecurrentDeliberation):
    def __init__(self, frozen_llm, inject_layers=(14, 16), rank=64, **kwargs):
...
inject_layers = [int(x) for x in '${COMBO}'.split(',')]
seed = $SEED
tag = f'dualL{inject_layers[0]}_{inject_layers[1]}_seed{seed}'
random.seed(seed)
torch.manual_seed(seed)
print(f'\n=== {tag} ===', flush=True)

controller = DualLayerDeliberation(
    frozen_llm=base_model, inject_layers=inject_layers, rank=64,
...
with open(os.path.join(RESULTS_DIR, f'{tag}.json'), 'w') as f:
    json.dump({'tag': tag, 'inject_layers': inject_layers, 'accuracy': acc, 'seed': seed}, f, indent=2)
