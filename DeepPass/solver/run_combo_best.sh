...
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_combo_best_%j.log
...
# Combine ALL the best ingredients: mid-layer L16 + train-5 + lowrank + GA16
# That's the killer combo
echo "=== ULTIMATE COMBO: mid-L16 + train-5 + lowrank built-in ==="
...
seed = $SEED
total_steps = 3000
tag = f'ultimate_midL16_train5_GA16_seed{seed}'
random.seed(seed)
...
for step in range(total_steps):
    ...
    all_cl, all_v = controller(prompt_emb, answer_emb, choice_ids_t, rounds=5)
...
with open(os.path.join(RESULTS_DIR, f'{tag}.json'), 'w') as f:
    json.dump({'tag': tag, 'config': 'midL16+train5+GA16+lowrank', 'seed': seed, 'results': results}, f, indent=2)
