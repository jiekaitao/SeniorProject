#!/bin/bash
#SBATCH --job-name=inst_ver
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_inst_verify_%j.log
...
# Verify Instruct L12 train-3 GA=8 across seeds
echo "=== Instruct L12 train-3 GA=8 verification (5 seeds) ==="
for L in 10 12 14; do
...
L = $L
seed = $SEED
tag = f'instructL{L}_verify_seed{seed}'
random.seed(seed)
...
with open(os.path.join(RESULTS_DIR, f'{tag}.json'), 'w') as f:
    json.dump({'tag': tag, 'model': 'Instruct', 'inject_layer': L, 'seed': seed, 'accuracy': acc}, f, indent=2)
