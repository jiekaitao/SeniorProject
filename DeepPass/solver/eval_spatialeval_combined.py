"""
Combined Sidecar + Prefix — Both memory injection channels simultaneously.

Sidecar alone = 71.8%, prefix alone = 72%. What if we use BOTH?
- Memory tokens prepended (prefix mode, position-based attention)
- Cross-attention sidecars at decoder layers (dedicated channel)

Two channels might provide complementary information pathways.
"""
...
def run_experiment(sidecar_layer_ids, d_bottleneck, seed, total_steps,
    ...
    tag = f'combined_L{layers_str}_d{d_bottleneck}_s{seed}'
    random.seed(seed)
    torch.manual_seed(seed)
    print(f'\n{"="*60}', flush=True)
    print(f'  Combined: sidecar layers={sidecar_layer_ids} + prefix, seed={seed}', flush=True)
    ...
    for step in range(total_steps):
        ...
        # COMBINED: memory prefix + sidecar injection
        dec_in = torch.cat([memory, all_emb], dim=1)  # prefix mode
        M = memory.shape[1]
        T = dec_in.shape[1]
        ...
        for i, layer in enumerate(lm_model.layers):
            ...
            if i in sidecar_set:
                h = h + sidecars[str(i)](h, memory)  # sidecar also injects
    ...
    result_data = {
        'tag': tag, 'method': 'combined_sidecar_prefix',
...
def main():
    ...
    layers = list(map(int, args.sidecar_layers.split(',')))
    ...
    print('\n=== All combined experiments complete ===', flush=True)
