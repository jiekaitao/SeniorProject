#!/bin/bash
#SBATCH --job-name=l12_t5
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_l12_train5_%j.log
...
# L12 with train-5 rounds (combine our two best findings)
echo "=== L12 with train-5 rounds + K-scaling eval ==="
...
for step in range(total_steps):
    ...
    all_cl, all_v = controller(prompt_emb, answer_emb, choice_ids_t, rounds=5)  # TRAIN 5
...
with open(os.path.join(RESULTS_DIR, f'{tag}.json'), 'w') as f:
    json.dump({'tag': tag, 'inject_layer': 12, 'train_rounds': 5, 'seed': seed, 'results': results}, f, indent=2)
