"""
SpatialEval with scaled solver — test if a bigger single solver breaks the 70% ceiling.
Uses the same bypass mode + Llama 3.1 8B decoder as v3, but with configurable solver width/depth.

Configurations:
  12M (baseline): d=512,  heads=8,  ffn=1024, L=2
  25M (wider):    d=768,  heads=12, ffn=1536, L=2
  42M (wide):     d=1024, heads=16, ffn=2048, L=2
  30M (deeper):   d=768,  heads=12, ffn=1536, L=4

Key question (from GPT-5.4 consultation):
  Does a larger monolithic solver beat the 70% ceiling?
  If YES → the bottleneck was solver capacity, not architecture
  If NO  → MoERM routing may be needed
"""
...
CONFIGS = {
    '12M': dict(d_model=512, n_heads=8, ffn_dim=1024, n_L_layers=2),
    '25M': dict(d_model=768, n_heads=12, ffn_dim=1536, n_L_layers=2),
    '42M': dict(d_model=1024, n_heads=16, ffn_dim=2048, n_L_layers=2),
    '30M_deep': dict(d_model=768, n_heads=12, ffn_dim=1536, n_L_layers=4),
}
...
def train_and_eval(config_name='25M', n_memory_slots=32, total_steps=2000, seed=42):
    cfg = CONFIGS[config_name]
    tag = f'scaled_{config_name}_mem{n_memory_slots}_s{seed}'

    random.seed(seed)
    torch.manual_seed(seed)

    print(f'=== Config: {config_name} | mem={n_memory_slots} | steps={total_steps} | seed={seed} ===', flush=True)
    ...
    solver = SolverCore(n_memory_slots=n_memory_slots, **cfg).to(device=device, dtype=torch.bfloat16)
    n_params = sum(p.numel() for p in solver.parameters())
    print(f'Solver: {n_params:,} params | d={cfg["d_model"]} heads={cfg["n_heads"]} ffn={cfg["ffn_dim"]} L={cfg["n_L_layers"]}', flush=True)
    ...
    for step in range(total_steps):
        ...
        if (step + 1) % 100 == 0:
            ...
            print(f'  step {step + 1} | loss={avg_loss:.4f} | K={K} | lr={lr:.6f} | {time.time() - t0:.0f}s', flush=True)
    ...
    print(f'\n=== Summary: {config_name} ({n_params/1e6:.1f}M params) ===', flush=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='25M', choices=list(CONFIGS.keys()))
    parser.add_argument('--mem', type=int, default=32)
    parser.add_argument('--steps', type=int, default=2000)
    parser.add_argument('--seed', type=int, default=42)
    args = parser.parse_args()
    train_and_eval(config_name=args.config, n_memory_slots=args.mem,
