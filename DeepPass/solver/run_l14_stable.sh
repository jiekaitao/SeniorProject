#!/bin/bash
#SBATCH --job-name=l14_stab
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_l14_stable_%j.log
...
echo "=== L14 stability test: 4 seeds + K-scaling ==="
...
seed = $SEED
total_steps = 3000
tag = f'L14_train5_GA16_seed{seed}'
random.seed(seed)
torch.manual_seed(seed)
print(f'\n=== {tag} ===', flush=True)

controller = MidLayerDeliberation(
    frozen_llm=base_model, inject_layer=14, rank=64,
...
with open(os.path.join(RESULTS_DIR, f'{tag}.json'), 'w') as f:
    json.dump({'tag': tag, 'inject_layer': 14, 'train_rounds': 5, 'GA': 16, 'seed': seed, 'results': results}, f, indent=2)
