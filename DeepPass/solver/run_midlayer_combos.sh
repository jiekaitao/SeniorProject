#!/bin/bash
#SBATCH --job-name=ml_combo
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_midlayer_combos_%j.log
...
# Test L16 mid-layer on cross-tasks (KEY EXPERIMENT)
echo "=== Mid-layer L16 on cross-tasks (the breakthrough test) ==="
...
# Custom run with task in tag
import time, json, math
import torch.nn as nn
import torch.nn.functional as F
from eval_deliberation_creative import MidLayerDeliberation, RESULTS_DIR, CHOICE_MAP

tag = f'midlayer_L16_${TASK}_r3_seed${SEED}'
random.seed($SEED)
torch.manual_seed($SEED)
print(f'\n=== {tag} ===', flush=True)

controller = MidLayerDeliberation(
    frozen_llm=base_model, inject_layer=16, rank=64,
...
for step in range(total_steps):
    ...
    prompt_enc = tokenizer(text, return_tensors='pt', truncation=True, max_length=1900, add_special_tokens=True).to(device)
...
for idx in eval_idx:
    ...
    text = sample['text'][:1500]
    oracle = sample['oracle_option'].strip().upper()
    answer_label_v = CHOICE_MAP.get(oracle[0], 0)
    prompt_enc = tokenizer(text, return_tensors='pt', truncation=True, max_length=1900, add_special_tokens=True).to(device)
    answer_enc = tokenizer('\nAnswer:', return_tensors='pt', add_special_tokens=False).to(device)
    with torch.no_grad():
        prompt_emb = lm_model.embed_tokens(prompt_enc['input_ids'])
    ...
    if pred == answer_label_v: correct += 1
acc = correct / n_eval
print(f'  FINAL {tag}: {acc:.4f} ({correct}/{n_eval})', flush=True)

os.makedirs(RESULTS_DIR, exist_ok=True)
with open(os.path.join(RESULTS_DIR, f'{tag}.json'), 'w') as f:
    json.dump({'tag': tag, 'task': '${TASK}', 'inject_layer': 16, 'accuracy': acc, 'correct': correct, 'total': n_eval, 'seed': $SEED}, f, indent=2)
