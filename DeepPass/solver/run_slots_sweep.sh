#!/bin/bash
#SBATCH --job-name=slots
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_slots_sweep_%j.log
...
# Try 16, 32 slots with L12 mid-layer + lowrank
echo "=== Bigger slots at L12 ==="
for N_SLOTS in 16 32; do
    for SEED in 42 7; do
        $PYTHON -c "
import sys, os
...
n_slots = $N_SLOTS
seed = $SEED
tag = f'midL12_slots{n_slots}_seed{seed}'
random.seed(seed)
...
controller = MidLayerDeliberation(
    ...
    d_state=512, n_slots=n_slots, tapped_layers=(8, 16, 24), topk_vocab=64,
...
with open(os.path.join(RESULTS_DIR, f'{tag}.json'), 'w') as f:
    json.dump({'tag': tag, 'inject_layer': 12, 'n_slots': n_slots, 'seed': seed, 'accuracy': acc}, f, indent=2)
