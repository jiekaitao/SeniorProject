...
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_midlayer_sweep_%j.log
...
echo "=== MID-LAYER SWEEP — finding optimal injection layer ==="
echo "L16 hit 73.0%, sweeping nearby layers"
$PYTHON solver/eval_deliberation_creative.py --experiment midlayer --seeds 42,7 --steps 3000

# Custom mid-layer sweep at more layers
echo ""
echo "=== Extra mid-layer values: 12, 14, 18, 20 ==="
for L in 12 14 18 20; do
    for SEED in 42 7; do
        $PYTHON -c "
import sys, os
os.environ['HF_HOME']='/blue/cis4914/jietao/hf_cache'
sys.path.insert(0, 'solver')
from eval_deliberation_creative import run_midlayer, get_choice_token_ids, MidLayerDeliberation, LowrankDeliberation
import torch, random
