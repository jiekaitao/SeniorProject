#!/bin/bash
#SBATCH --job-name=inst_psh
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_instruct_push_%j.log
...
# L12 on instruct with more seeds + train-5 + GA16 + cross-task
echo "=== Push Llama Instruct L12 further ==="
...
seed = $SEED
total_steps = 3000
tag = f'instructL12_train5_${TASK}_seed{seed}'
random.seed(seed)
...
for er in [3, 5, 8]:
    ...
    results[f'rounds={er}'] = {'accuracy': acc}
    print(f'  rounds={er}: {acc:.4f}', flush=True)

os.makedirs(RESULTS_DIR, exist_ok=True)
with open(os.path.join(RESULTS_DIR, f'{tag}.json'), 'w') as f:
    json.dump({'tag': tag, 'model': 'Instruct', 'task': '${TASK}', 'inject_layer': 12, 'seed': seed, 'results': results}, f, indent=2)
