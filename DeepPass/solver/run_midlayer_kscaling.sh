#!/bin/bash
#SBATCH --job-name=ml_kscale
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_midlayer_kscaling_%j.log
...
echo "=== Mid-layer L16 K-scaling: train with N rounds, eval at 1-10 ==="
for TRAIN_R in 3 5; do
    for SEED in 42 7; do
        $PYTHON -c "
import sys, os
...
seed = $SEED
total_steps = 3000
tag = f'midlayer_L16_kscale_train{train_r}_seed{seed}'
random.seed(seed)
...
for step in range(total_steps):
    ...
    all_cl, all_v = controller(prompt_emb, answer_emb, choice_ids_t, rounds=train_r)
...
with open(os.path.join(RESULTS_DIR, f'{tag}.json'), 'w') as f:
    json.dump({'tag': tag, 'inject_layer': 16, 'train_rounds': train_r, 'seed': seed, 'results': results}, f, indent=2)
