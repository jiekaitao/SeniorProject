...
#SBATCH --qos=cis4914
#SBATCH --output=/blue/cis4914/jietao/DeepPass/results/sbatch_expa_k3_%j.log
#SBATCH --job-name=expa
cd /blue/cis4914/jietao/DeepPass
export LD_PRELOAD=/blue/cis4914/jietao/DeepPass/envs/deeppass/lib/libstdc++.so.6
echo "=== Exp A K=3 (layers 12-15, 2 replay passes) ==="
...
model = BandRecurrentLlama(base_model, replay_layer_ids=(12,13,14,15), lora_rank=16, max_extra_passes=2)
...
for batch in data_stream:
    ...
    _, loss = model(batch[:, :-1], labels=batch[:, :-1], K=3, hard_frac=0.25)
    ...
    if step % 500 == 0:
        results = {}
        for K in [1, 2, 3]:
            ppl_data = evaluate(model, tokenizer, device, n_batches=10, seq_len=1024, bs=2)
            # evaluate only does K=1,2 — run model manually for K=3
