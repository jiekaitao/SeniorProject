...
class RNATRMTrainer(pl.LightningModule):

    def __init__(self, cfg_train, cfg_model, py_logger, data_module):
        super().__init__()
        ...
        self.model = RiboFormerTRM(cfg_model)
        ...
        if not hasattr(cfg_model, "binary_output") or cfg_model.binary_output:
            ...
            self.loss_train = torch.nn.CrossEntropyLoss(
                ignore_index=self.ignore_index, reduction='none', label_smoothing=0.0)
            self.loss_valid = torch.nn.CrossEntropyLoss(
                ignore_index=self.ignore_index, reduction='none', label_smoothing=0.0)

        # Phase 3: ACT loss (optional)
        if self.model.act_enabled:
            self.act_loss = RNAACTLoss(
                enable_brier_halting=getattr(cfg_model, 'enable_brier_halting', False),
                enable_monotonicity=getattr(cfg_model, 'enable_monotonicity', False),
                enable_smoothness=getattr(cfg_model, 'enable_smoothness', False),
                supervision_decay=getattr(cfg_model, 'supervision_decay', 0.7),
            )
    ...
    def training_step(self, batch, batch_idx):
        # CGAR: set curriculum depth based on training progress
        if self.model.cgar_enabled:
            progress = self.current_epoch / max(1, self.trainer.max_epochs)
            self.model.set_curriculum_depth(progress)

        # Original cycling curriculum
        if self.model.cycling:
            max_cycle = min(max(1, self.current_epoch // 3), self.model.cycle_steps)
        else:
            max_cycle = 0

        result = self.model(batch['src_seq'], batch['length'], batch['pdb_sample'],
                            max_cycle=max_cycle)

        if self.model.act_enabled:
            logits_mat, mask, q_halt = result
            # Use ACT loss
            total_loss, metrics = self.act_loss(
                logits_mat, batch['trg_mat'], batch['mask'], q_halt_logit=q_halt)
            loss = total_loss
        else:
            logits_mat, mask = result
        ...
        with torch.no_grad():
            ...
            for b, length in enumerate(batch['length'].detach().cpu().numpy()):
                ...
                metrics['solved'].append(
                    torch.tensor([solved], dtype=true_mat.dtype, device=true_mat.device,
                                 requires_grad=False))

                tp = torch.logical_and(pred_mat, true_mat).sum()
                tn = torch.logical_and(torch.logical_not(pred_mat),
                                       torch.logical_not(true_mat)).sum()
                ...
                mcc = (tp * tn - fp * fn) / (
                    1e-4 + torch.sqrt((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn)))
            ...
            self.log(
                f"val/{self.val_sets_name[dataloader_idx]}/loss", loss,
                on_step=False, on_epoch=True,
                prog_bar=dataloader_idx is None or dataloader_idx == 0,
                sync_dist=True)
    ...
    def configure_optimizers(self):
        parameters = group_parameters_for_optimizer(
            self.model, self.cfg_train.optimizer,
